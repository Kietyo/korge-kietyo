...
open class CoreGraphicsImageFormatProvider : AwtNativeImageFormatProvider() {
  ...
  object CoreFoundation {
    // CFNumberGetValue(number: platform.CoreFoundation.CFNumberRef? /* =
    // kotlinx.cinterop.CPointer<cnames.structs.__CFNumber>? */, theType:
    // platform.CoreFoundation.CFNumberType /* = kotlin.Int */, valuePtr:
    // kotlinx.cinterop.CValuesRef<*>?): kotlin.Boolean { /* compiled code */ }
    ...
    val kCFBooleanFalse: Pointer?
      get() = lib.getGlobalVariableAddress("kCFBooleanFalse")
    val kCFBooleanTrue: Pointer?
      get() = lib.getGlobalVariableAddress("kCFBooleanTrue")
    // val kCFNumberIntType: Pointer? get() = lib.getGlobalVariableAddress("kCFNumberIntType")
    val kCFNumberIntType: Int
      get() = 9

    @JvmStatic
    external fun CFDataCreate(allocator: Pointer?, bytes: Pointer?, length: Int): Pointer?
    @JvmStatic
    external fun CFDataCreate(allocator: Pointer?, bytes: ByteArray, length: Int): Pointer?
    @JvmStatic external fun CFDataGetBytePtr(data: Pointer?): Pointer?
    @JvmStatic
    external fun CFDictionaryCreateMutable(
        allocator: Pointer?,
        capacity: Int,
    ...
    @JvmStatic
    external fun CFDictionaryAddValue(theDict: Pointer?, key: Pointer?, value: Pointer?): Unit
  ...
  object CoreGraphics {
    ...
    val kCGImageAlphaNone: Int
      get() = 0
    val kCGImageAlphaPremultipliedLast: Int
      get() = 1
    val kCGImageAlphaPremultipliedFirst: Int
      get() = 2
    val kCGImageAlphaLast: Int
      get() = 3
    val kCGImageAlphaFirst: Int
      get() = 4
    ...
    @JvmStatic
    external fun CGBitmapContextCreate(
        buffer: Pointer?,
        width: Int,
        height: Int,
        bits: Int,
    ...
    @JvmStatic
    external fun CGContextDrawImage(context: Pointer?, rect: CGRect.ByValue, cgImage: Pointer?)
    init {
      Native.register(LIB)
    }
  }
  object ImageIO {
    ...
    val lib: NativeLibrary =
        NativeLibrary.getInstance("/System/Library/Frameworks/ImageIO.framework/Versions/A/ImageIO")
    val kCGImageSourceShouldCache: Pointer?
      get() = lib.getGlobalVariableAddress("kCGImageSourceShouldCache")
    val kCGImageSourceCreateThumbnailWithTransform: Pointer?
      get() = lib.getGlobalVariableAddress("kCGImageSourceCreateThumbnailWithTransform")
    val kCGImageSourceCreateThumbnailFromImageAlways: Pointer?
      get() = lib.getGlobalVariableAddress("kCGImageSourceCreateThumbnailFromImageAlways")
    val kCGImagePropertyPixelWidth: Pointer?
      get() = lib.getGlobalVariableAddress("kCGImagePropertyPixelWidth")
    val kCGImagePropertyPixelHeight: Pointer?
      get() = lib.getGlobalVariableAddress("kCGImagePropertyPixelHeight")

    @JvmStatic external fun CGImageSourceCreateWithData(data: Pointer?, options: Pointer?): Pointer?
    @JvmStatic
    external fun CGImageSourceCreateImageAtIndex(
        imgSource: Pointer?,
        index: Int,
        dict: Pointer?
    ): Pointer?
    @JvmStatic
    external fun CGImageSourceCopyPropertiesAtIndex(
        imgSource: Pointer?,
        index: Int,
        dict: Pointer?
    ): Pointer?
  ...
  override suspend fun decodeHeaderInternal(data: ByteArray): ImageInfo = nsAutoreleasePool {
    ...
    val props =
        ImageIO.CGImageSourceCopyPropertiesAtIndex(imgSource, 0, null)
            ?: error("Failed trying to read image in decodeHeaderInternal")

    ImageInfo(
        getIntFromDict(props, ImageIO.kCGImagePropertyPixelWidth),
        getIntFromDict(props, ImageIO.kCGImagePropertyPixelHeight))
  }

  private fun getIntFromDict(props: Pointer?, key: Pointer?): Int {
    val mem = Memory(8)
    CoreFoundation.CFNumberGetValue(
        CoreFoundation.CFDictionaryGetValue(props, key), CoreFoundation.kCFNumberIntType, mem)
    return mem.getInt(0L)
  }

  override suspend fun decodeInternal(
      data: ByteArray,
      props: ImageDecodingProps
  ): NativeImageResult {
    val (res, time) =
        measureTimeWithResult { withContext(Dispatchers.IO) { _decodeInternal(data, props) } }
  ...
  private suspend fun _decodeInternal(
      data: ByteArray,
      props: ImageDecodingProps
  ): NativeImageResult {
    ...
    return nsAutoreleasePool {
      ...
      CoreFoundation.CFDictionaryAddValue(
          dict,
          ImageIO.kCGImageSourceShouldCache!!.getPointer(0L),
          CoreFoundation.kCFBooleanFalse!!.getPointer(0L))
      CoreFoundation.CFDictionaryAddValue(
          dict,
          ImageIO.kCGImageSourceCreateThumbnailWithTransform!!.getPointer(0L),
          CoreFoundation.kCFBooleanFalse!!.getPointer(0L))
      CoreFoundation.CFDictionaryAddValue(
          dict,
          ImageIO.kCGImageSourceCreateThumbnailFromImageAlways!!.getPointer(0L),
          CoreFoundation.kCFBooleanTrue!!.getPointer(0L))
      ...
      val context =
          CoreGraphics.CGBitmapContextCreate(
              pixels, width, height, 8, (width * 4), colorSpace, alphaInfo)
      ...
      val bufferedImage =
          BufferedImage(
              width,
              height,
              if (props.asumePremultiplied || realPremultiplied) BufferedImage.TYPE_INT_ARGB_PRE
      ...
      // val bitmap = Bitmap32(width, height, premultiplied = true).also { bmp -> pixels!!.read(0L,
      // bmp.ints, 0, width * height) }
