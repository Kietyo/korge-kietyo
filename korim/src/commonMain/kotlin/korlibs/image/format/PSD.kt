...
object PSD : ImageFormat("psd") {
  ...
  private fun FastByteArrayInputStream.readImageData(header: PsdImageInfo): Bitmap32 {
    ...
    when (compressionMethod) {
      ...
      1 -> { // RLE
        ...
        // println("PSD: channels=${header.channels}, bitsPerChannel=${header.bitsPerChannel},
  ...
  override fun decodeHeader(s: SyncStream, props: ImageDecodingProps): PsdImageInfo? =
      s.run {
        ...
        @Suppress("UNUSED_VARIABLE") val reserved = readBytes(6)
        val channels = readU16BE()
        val height = readS32BE()
        val width = readS32BE()
        val bitsPerChannel = readU16BE()
        val colorMode = readU16BE()
        return PsdImageInfo(width, height, bitsPerChannel * channels, channels, bitsPerChannel, colorMode)
      }
}
