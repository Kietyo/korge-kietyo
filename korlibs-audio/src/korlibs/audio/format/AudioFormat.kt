...
open class AudioFormat(vararg exts: String) {
    ...
    open suspend fun tryReadInfo(data: AsyncStream, props: AudioDecodingProps = AudioDecodingProps.DEFAULT): Info? =
        null

    protected open suspend fun decodeStreamInternal(
        data: AsyncStream,
        props: AudioDecodingProps = AudioDecodingProps.DEFAULT
    ): AudioStream? = null
    ...
    suspend fun decode(data: AsyncStream, props: AudioDecodingProps = AudioDecodingProps.DEFAULT): AudioData? =
        decodeStream(data, props)?.toData(props.maxSamples)

    suspend fun decode(data: ByteArray, props: AudioDecodingProps = AudioDecodingProps.DEFAULT): AudioData? =
        decodeStream(data.openAsync(), props)?.toData(props.maxSamples)
...
class AudioFormats : AudioFormat() {
    ...
    fun registerFirst(vararg formats: AudioFormat): AudioFormats =
        invalidate().apply { this.formats.addAll(0, formats.toList()) }
...
suspend fun VfsFile.readSoundInfo(
    formats: AudioFormat = defaultAudioFormats,
    props: AudioDecodingProps = AudioDecodingProps.DEFAULT
) =
