...
class KorgePluginsContainer(
    val project: Project,
    val parentClassLoader: ClassLoader = KorgePluginsContainer::class.java.classLoader
) {
...
data class KorgePluginDescriptor(
    val container: KorgePluginsContainer,
    val artifact: MavenLocation,
    ...
    fun addArgs(args: Map<String, String>) =
        this.apply { this.args.putAll(args) }.apply { container.globalParams.putAll(args) }
}

data class MavenLocation(val group: String, val name: String, val version: String, val classifier: String? = null) {
    val versionWithClassifier by lazy {
        buildString {
            append(version)
            if (classifier != null) {
                append(':')
...
open class KorgeExtension(
    ...
    companion object {
        ...
        fun isIdValid(id: String) =
            id.isNotEmpty() && id.isNotBlank() && id.split(".").all { it.matches(validIdentifierRegexp) }
    ...
    fun checkVersion(
        check: Boolean = true,
        report: Boolean = true,
        telemetry: Boolean = project.findProperty("korge.disable.telemetry") != "true"
    ) {
    ...
    @Deprecated("", ReplaceWith("targetAndroid()"))
    fun targetAndroidIndirect() = targetAndroid()
    @Deprecated("", ReplaceWith("targetAndroid()"))
    fun targetAndroidDirect() = targetAndroid()
    ...
    fun serializationJson() {
        ...
        project.dependencies.add(
            "commonMainApi",
            "org.jetbrains.kotlinx:kotlinx-serialization-json:${BuildVersions.KOTLIN_SERIALIZATION}"
        )
    ...
    val realEntryPoint: String
        get() = entryPoint ?: (jvmMainClassName.substringBeforeLast(
            '.',
            ""
        ) + ".main").trimStart('.')
    ...
    fun dependencyMulti(
        ...
        name: String,
        version: String,
        targets: List<String> = ALL_TARGETS,
        suffixCommonRename: Boolean = false,
        androidIsJvm: Boolean = false
    ): Unit = project {
